---
created: 2025-02-17T15:06:25 (UTC -08:00)
tags: []
source: https://iiif.io/api/search/2.0/
author: 
---

# IIIF Content Search API 2.0 — IIIF | International Image Interoperability Framework
...
---
## Status of this Document[](https://iiif.io/api/search/2.0/#status-of-this-document)

**This Version:** 2.0.0

**Latest Stable Version:** [2.0.0](https://iiif.io/api/search/ "Latest Content Search API Stable Version")

**Previous Version:** [1.0.0](https://iiif.io/api/search/1.0/)

**Editors**

-   **[Michael Appleby](https://orcid.org/0000-0002-1266-298X)**, Yale University
    
-   **[Dawn Childress](https://orcid.org/0000-0003-2602-2788)**, UCLA
    
-   **[Tom Crane](https://orcid.org/0000-0003-1881-243X)**, Digirati
    
-   **[Jeff Mixter](https://orcid.org/0000-0002-8411-2952)**, OCLC
...
We welcome [feedback](https://iiif.io/api/#feedback "IIIF APIs: Feedback requested") on all IIIF Specifications.

### 1.1. Use Cases[](https://iiif.io/api/search/2.0/#use-cases)
...
### 1.2. Terminology[](https://iiif.io/api/search/2.0/#terminology)
...
### 1.3. Common Specification Features[](https://iiif.io/api/search/2.0/#common)
...
## 3\. Declaring Services[](https://iiif.io/api/search/2.0/#3-declaring-services)

The Content Search and the Autocomplete services are associated with IIIF resources using the `service` property, defined by the [Presentation API](https://iiif.io/api/presentation/3.0/#service) as an array of JSON objects. These objects _must_ have the `id` and `type` properties. The value of the `id` property _must_ be the URI used to interact with the service.

Any resource in the [Presentation API](https://iiif.io/api/presentation/3.0/ "Presentation API") _may_ have a Content Search service associated with it. The resource determines the scope of the content that will be searched. A service associated with a Manifest will search all of the annotations on Canvases or other resources below the Manifest, a service associated with a particular Range will only search the Canvases within the Range, or a service on a Canvas will search only Annotations on that particular Canvas.
...
## 4\. Content Search[](https://iiif.io/api/search/2.0/#search)
...
### 4.1. Request[](https://iiif.io/api/search/2.0/#request-1)
...
#### 4.1.1. Query Parameters[](https://iiif.io/api/search/2.0/#query-parameters)
...
| `motivation` | A space separated list of motivation terms. If multiple motivations are supplied, an annotation matches the search if any of the motivations are present. Common values for the motivation parameter can be found in the [IIIF Registry of Motivations](https://iiif.io/api/registry/motivations/), including the two Content Search motivations `contextualizing` and `highlighting` defined in sections [4.3.1](https://iiif.io/api/search/2.0/#match-context) and [4.3.2](https://iiif.io/api/search/2.0/#match-highlighting) below. |
...
### 4.2. Responses[](https://iiif.io/api/search/2.0/#responses)

The response from the server _must_ be an [Annotation Page](https://iiif.io/api/presentation/3.0/#55-annotation-page), following the format from the Presentation API with some additional features. This allows clients that already implement the Annotation Page format to avoid further implementation work to support search results.

#### 4.2.1. Simple Lists[](https://iiif.io/api/search/2.0/#simple-lists)
...
#### 4.2.2. Paging Results[](https://iiif.io/api/search/2.0/#paging-results)
...
#### 4.2.3. References to Containing Resources[](https://iiif.io/api/search/2.0/#references-containing-resources)
...
#### 4.2.4. Ignored Parameters[](https://iiif.io/api/search/2.0/#ignored-parameters)
...
#### 4.3.1. Match Context[](https://iiif.io/api/search/2.0/#match-context)
...
To meet this requirement, the Annotations have a `motivation` of `contextualizing`, and a [Web Annotation Data Model](https://www.w3.org/TR/annotation-model/#text-quote-selector "Web Annotation Model - Text Quote Selector") `TextQuoteSelector` with `prefix` and `suffix` of the text immediately before and after the matching content in the annotation. The matching content is conveyed in the `exact` property. The selector has the URI of the annotation it refers to in the `source` property, to be matched against the `id` property of the annotations in `items`.
...
#### 4.3.2. Match Highlighting[](https://iiif.io/api/search/2.0/#match-highlighting)
...
#### 4.3.3. Multi-Match Annotations[](https://iiif.io/api/search/2.0/#multi-match-annotations)
...
#### 4.3.4. Multi-Annotation Matches[](https://iiif.io/api/search/2.0/#multi-annotation-matches)
...
## 5\. Autocomplete[](https://iiif.io/api/search/2.0/#autocomplete)
...
### 5.1. Autocomplete Request[](https://iiif.io/api/search/2.0/#request)
...
### 5.2. Autocomplete Response[](https://iiif.io/api/search/2.0/#autocomplete-response)
...
### 5.3. Extended Autocomplete Responses[](https://iiif.io/api/search/2.0/#extended-autocomplete-response)
...
-   `label` - The `Term` _may_ have a `label` property. The value is a JSON object which follows the definition of a [language map](https://iiif.io/api/presentation/3.0/#language-of-property-values "Language of Property Values"). This label should be displayed to the user instead of the `value`, for example when the `value` is a URI or a string that has been manipulated with stemming or other normalization.
-   `language` - The `Term` _may_ have a `language` property. The value is a string conforming to the [BCP 47](https://tools.ietf.org/html/bcp47 "Tags for Identifying Languages") language code specification, and gives the language of the term string in the `value` property.
...
## Appendices[](https://iiif.io/api/search/2.0/#appendices)

### A. Versioning[](https://iiif.io/api/search/2.0/#a-versioning)

This specification follows [Semantic Versioning](http://semver.org/spec/v2.0.0.html "Semantic Versioning 2.0.0"). See the note [Versioning of APIs](https://iiif.io/api/annex/notes/semver/ "Versioning of APIs") for details regarding how this is implemented.

### B. Acknowledgements[](https://iiif.io/api/search/2.0/#b-acknowledgements)

Many thanks to the members of the [IIIF](https://iiif.io/community/ "IIIF Community") for their continuous engagement, innovative ideas and feedback.
...
### C. Change Log[](https://iiif.io/api/search/2.0/#c-change-log)

| Date | Description |
| --- | --- |
| 2022-11-15 | Version 2.0 (Mr. Wigglesworth) [View change log](https://iiif.io/api/search/2.0/change-log/ "Content Search API 2.0 Change Log") |
