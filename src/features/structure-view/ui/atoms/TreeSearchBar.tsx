    ...
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="relative">
        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          <svg className={cn('h-4 w-4', cx?.textMuted ?? 'text-nb-black/40')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {showMatchInfo && (
        <div className="flex justify-between items-center text-sm px-1">
          <span className={cn('font-serif', cx?.textMuted ?? 'text-nb-black/50')}>
            {matchCount === 0 ? (
              'No matches found'
            ) : (
              <>
                <span className={cn('font-medium', cx?.text ?? 'text-nb-black/20')}>{matchCount}</span>
                {' '}of{' '}
                <span className={cn(cx?.textMuted ?? 'text-nb-black/50')}>{totalCount}</span>
                {' '}items
              </>
            )}
          </span>
