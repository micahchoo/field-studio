...
// ═══════════════════════════════════════════════════════════════════════
// 14. Tile Pipeline — planTileGeneration
// ═══════════════════════════════════════════════════════════════════════
...
import type { TileManifest } from '@/src/shared/types/image-pipeline';

describe('tilePipeline', () => {
  describe('planTileGeneration', () => {
    it('returns skipped for images below minDimensionForTiling', () => {
      const result = planTileGeneration('asset-1', 200, 200, {
        ...DEFAULT_TILE_CONFIG,
        minDimensionForTiling: 500,
      });
      expect(result.coords).toHaveLength(0);
      expect(result.manifest.status).toBe('skipped');
      expect(result.manifest.totalTiles).toBe(0);
    });

    it('plans tiles for a 4096x4096 image with defaults', () => {
      const result = planTileGeneration('asset-1', 4096, 4096);
      expect(result.coords.length).toBeGreaterThan(0);
      expect(result.manifest.status).toBe('pending');
      expect(result.manifest.totalTiles).toBe(result.coords.length);
      expect(result.manifest.assetId).toBe('asset-1');
      expect(result.manifest.width).toBe(4096);
      expect(result.manifest.height).toBe(4096);
    });

    it('returns skipped when tiles exceed maxTilesPerImage', () => {
      const result = planTileGeneration('asset-1', 100000, 100000, {
        ...DEFAULT_TILE_CONFIG,
        maxTilesPerImage: 10,
      });
      expect(result.manifest.status).toBe('skipped');
    ...
    it('generates correct manifest metadata', () => {
      const config = { ...DEFAULT_TILE_CONFIG, format: 'webp' as const };
      const result = planTileGeneration('asset-1', 2048, 1024, config);
      expect(result.manifest.tileSize).toBe(config.tileSize);
      expect(result.manifest.format).toBe('webp');
    });

    it('scaleFactors start at 1 and double', () => {
      const result = planTileGeneration('asset-1', 4096, 4096);
      expect(result.scaleFactors[0]).toBe(1);
      for (let i = 1; i < result.scaleFactors.length; i++) {
        expect(result.scaleFactors[i]).toBe(result.scaleFactors[i - 1] * 2);
      }
    });

    it('tile coords cover all levels', () => {
      const result = planTileGeneration('asset-1', 2048, 2048);
      const levels = new Set(result.coords.map((c) => c.level));
      expect(levels.size).toBe(result.scaleFactors.length);
    });

    it('totalSize starts at 0', () => {
      const result = planTileGeneration('asset-1', 2048, 2048);
      expect(result.manifest.totalSize).toBe(0);
    });
  });

  // ═══════════════════════════════════════════════════════════════════════
  // 15. Tile Pipeline — generateInfoJson
  // ═══════════════════════════════════════════════════════════════════════

  describe('generateInfoJson', () => {
    it('generates valid IIIF Image API 3.0 info.json', () => {
    ...
    it('generates sizes for non-full-resolution scale factors', () => {
      const manifest: TileManifest = {
        assetId: 'asset-1',
        width: 4096,
        height: 3072,
      ...
      const info = generateInfoJson('https://iiif.example.org', 'asset-1', manifest);
      // Factor 1 is skipped; factors 2, 4, 8 create sizes
      expect(info.sizes).toHaveLength(3);
      expect(info.sizes![0]).toEqual({ width: 2048, height: 1536 });
      expect(info.sizes![1]).toEqual({ width: 1024, height: 768 });
      expect(info.sizes![2]).toEqual({ width: 512, height: 384 });
    });

    it('generates tiles array with scale factors', () => {
      const manifest: TileManifest = {
        assetId: 'asset-1',
        width: 2048,
        height: 2048,
      ...
      expect(info.tiles![0].width).toBe(512);
      expect(info.tiles![0].scaleFactors).toEqual([1, 2, 4]);
    });

    it('omits sizes and tiles when scaleFactors is empty', () => {
      const manifest: TileManifest = {
        assetId: 'asset-1',
        width: 256,
        height: 256,
      ...
      const info = generateInfoJson('https://iiif.example.org', 'asset-1', manifest);
      expect(info.sizes).toBeUndefined();
      expect(info.tiles).toBeUndefined();
    });

    it('maps jpeg format to jpg in preferredFormats', () => {
      const manifest: TileManifest = {
        assetId: 'a',
        width: 100,
        height: 100,
      ...
      const info = generateInfoJson('https://iiif.example.org', 'a', manifest);
      expect(info.preferredFormats).toEqual(['jpg']);
    });

    it('preserves webp format in preferredFormats', () => {
      const manifest: TileManifest = {
        assetId: 'a',
        width: 100,
        height: 100,
      ...
      const info = generateInfoJson('https://iiif.example.org', 'a', manifest);
      expect(info.preferredFormats).toEqual(['webp']);
    });

    it('includes extraQualities and extraFeatures', () => {
      const manifest: TileManifest = {
        assetId: 'a',
        width: 1000,
        height: 1000,
      ...
      const info = generateInfoJson('https://iiif.example.org', 'a', manifest);
      expect(info.extraQualities).toContain('gray');
      expect(info.extraQualities).toContain('bitonal');
      expect(info.extraFeatures).toContain('regionByPx');
      expect(info.extraFeatures).toContain('sizeByW');
      expect(info.extraFeatures).toContain('sizeByH');
      expect(info.extraFeatures).toContain('sizeByWh');
      expect(info.extraFeatures).toContain('baseUriRedirect');
    });
  });

  // ═══════════════════════════════════════════════════════════════════════
  // 16. Tile Pipeline — shouldGenerateTiles
  // ═══════════════════════════════════════════════════════════════════════

  describe('shouldGenerateTiles', () => {
    it('returns false when autoGenerate is disabled', () => {
      expect(shouldGenerateTiles(8000, 6000, {
        ...DEFAULT_TILE_CONFIG,
        autoGenerate: false,
      })).toBe(false);
    });

    it('returns false for images below minDimensionForTiling', () => {
      expect(shouldGenerateTiles(200, 200, {
        ...DEFAULT_TILE_CONFIG,
        minDimensionForTiling: 500,
      })).toBe(false);
    });

    it('returns true for eligible images', () => {
      expect(shouldGenerateTiles(4096, 3072, DEFAULT_TILE_CONFIG)).toBe(true);
    });

    it('returns false when estimated tiles exceed maxTilesPerImage', () => {
      expect(shouldGenerateTiles(100000, 100000, {
        ...DEFAULT_TILE_CONFIG,
        maxTilesPerImage: 10,
      })).toBe(false);
    });

    it('returns true for image exactly at minDimensionForTiling', () => {
      const minDim = DEFAULT_TILE_CONFIG.minDimensionForTiling;
      expect(shouldGenerateTiles(minDim, 100, DEFAULT_TILE_CONFIG)).toBe(true);
    });

    it('returns false when max dimension is 1 pixel below threshold', () => {
      const minDim = DEFAULT_TILE_CONFIG.minDimensionForTiling;
      expect(shouldGenerateTiles(minDim - 1, minDim - 1, DEFAULT_TILE_CONFIG)).toBe(false);
    });
  });
});
